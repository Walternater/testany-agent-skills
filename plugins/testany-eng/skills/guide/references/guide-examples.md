...
以下正式流程示例中的 approved 均以可追溯批准来源为前提，不靠标题或状态词自证；有限修复不强制走正式文档主线。
...
## 示例 13：跨三仓的 SQL 修复方案不是 HLD

证据：现有批准 ADR 固定职责/权限，修复只改变 nullable 参数绑定，尚无实现；提交的是一页计划，无新 Manifest。

推荐 `/lld-reviewer` 的 `bounded_change`：核查两条 SQL 的等价性、事务及真实数据库回归。无需补新 PRD/HLD、LLD Manifest 或 Runbook。若用户交的是精确实现，改走 `/code-reviewer`。

## 示例 14：混合修复不整包升级

证据：计划 A 修正事务；计划 B 把后台机器同步改成每页必须通过用户 PDP，旧基线没有此依赖，作者称“复用更安全”。

推荐：A 继续有限 LLD；B 单独 `/hld-reviewer` 评估职责/信任/故障依赖增量，并核实有权 Owner 的批准；权限契约改变时只审对应 API 增量。不能把 B 写成必须整改的 Java 参数问题，也不能因 B 待决让 A 停工。

## 示例 15：别把“纠偏”变成反向越界

证据：既有用户决定明确要求机器任务使用 PDP；修复只让已批准的 service principal 输入符合既有策略，未新增规则。

推荐有限 `/lld-reviewer`，保留该 PDP。是否机器任务不是结论，原始批准边界才是依据；不应套用上例要求移除授权检查。

## 示例 16：批准污染不由文件名洗白

证据：`approved-hld.md` 唯一依据是 Reviewer 曾写“P1 加新服务”；无 Owner 授权；用户只问评审方案。

不能将其标为有效 approved 并直接路由实现。回溯原始基线，向适当 Owner 说明职责/成本变化，保留独立已批准范围。无需新建审批系统来解决一次来源缺口。

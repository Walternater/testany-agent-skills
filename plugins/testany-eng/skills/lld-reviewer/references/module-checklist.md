...
本文档保留正式 LLD 的模块覆盖清单，适用方式由 `SKILL.md` 的模式及 `../../../references/review-boundaries.md` 决定。

- `formal_design`：按 Manifest 审查 Included 模块及 Excluded 理由，不省略已要求的正式覆盖。
- `bounded_change`：只取受影响的设计问题；既有未变部分可以直接引用，不要求补齐整份 Manifest 或另写 Test Strategy/Test Spec/Runbook。
- 下表“风险提示”仅说明该设计错误可能产生的风险，不是缺一章节便自动判 P0/P1。必要事实缺失是 Evidence gap；具体违反有效要求才按实际失败/影响定严重度。P2 始终可选。
- 清单不是增加能力的授权。DLQ、Feature Flag、PDP、Dashboard、ledger、平台服务等仅在已批准范围内检查；不能因为命中 Async/Security/Deployment 模块就要求新增。必要架构增量按 Scope decision 单列。

## Core 模块（必选）

正式 LLD 必须表达 Core 设计；有限修复只需把受影响方法、数据、流程和验证说清楚，形式可以是伪代码、SQL、时序或等价精确描述。
...
- [ ] 是否覆盖主要 Happy Path？
- [ ] 是否覆盖关键异常分支？
- [ ] 伪代码/SQL/等价流程是否可读且无关键歧义？
- [ ] 复杂时序是否需要图帮助理解？不因无图单独阻断。
...
| 3 | 幂等处理 | 幂等键、去重策略 | P1 |
| 4 | 失败终态与恢复 | 已批准的重试/失败处理；仅既有或已批准 DLQ 才检查其处理、告警 | P1 |
...
- [ ] 幂等键是否定义？
- [ ] 失败重试策略是否明确？
- [ ] 已批准的失败终态/恢复流程是否明确？若采用 DLQ 再检查其处理，不自动新增队列。
...
- [ ] 机器任务/人的行为是否仍使用其已批准授权职责？不能以最小权限为名新增用户依赖，也不能以收敛为名删除原有检查。
- [ ] 敏感数据是否有保护策略？
- [ ] 是否满足合规要求（如有）？
...
| 2 | 回滚策略 | 回滚条件与步骤 | P1 |
| 3 | 兼容/启用边界 | 沿用已批准机制；采用 Feature Flag 时说明策略与影响范围 | P1 |

### 检查要点

- [ ] 发布步骤是否可执行？
- [ ] 回滚是否可行且具备触发条件？
- [ ] 既定启用/兼容策略是否成立？仅已有或获批开关才检查 Feature Flag，不自动要求双轨。
...
- [ ] 认证/限流是否明确？
- [ ] 重试/降级是否有策略？
- [ ] 真实管理入口/生产 SDK 能否表达和提交拟议配置？自写 compiler 或 mock 管理 API 不证明该边界；列清调用输入、管理入口、执行器与结果各自证据。
...
正式 Manifest 将模块标为 Excluded 时，需说明适用性理由。有限修复可说明“本次不改，沿用已批准设计/现有能力”，不要求重新为整个系统填写 N/A 表。
